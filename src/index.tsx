...
export const CommandRouter = (props: CommandRouterProps) => {
  ...
  const matchedCommandElement = children.find(c => Command === c.type && c.props.name === command)

  const noMatchElement =
    children.find(c => NoMatch === c.type) !== undefined ? children.find(c => NoMatch === c.type) : <DefaultNoMatch />
